#!/usr/bin/env node
/**
 * Section 5 — final-technical-check.yml migrated from a direct
 ...
 * user-content concatenation (final scripts + linked QC report), the
 * business-profile-driven max token budget/model, the
 * FINAL_TECHNICAL_AI_USAGE_V1 usage marker (now reporting the ACTUAL
 * provider/model used), the heading-contract check, the downstream
 * final_technical_decision_contract.mjs call, the TEST_MODE fail-closed
 * gate, and — since ai_router.py's usable() accepts a broader set of stop
 * reasons than this workflow's own exact `stop_reason == "end_turn"`
 * check — an explicit post-router strict end_turn re-check. The old
 * single-key presence guard is deliberately dropped, since failing closed
 * just because ONE specific provider's key is absent would defeat the
 * router's whole multi-provider fallback purpose.
 */
...
const workflow = read(".github/workflows/final-technical-check.yml");
...
// 3. Request-prep step still writes the same system prompt file, and now
// writes only the user-content prompt as plain text.
const prepStepIdx = mustFind(workflow, "- name: Son kontrol isteğini hazırla", "request-prep step", validateIdx);
const aiStepIdx = mustFind(workflow, "- name: AI Router ile tek çağrıda son teknik kontrol yap", "AI-calling step", prepStepIdx);
...
for (const systemFragment of [
  "Sen $BRAND_NAME için çalışan bağımsız son teknik kontrol editörüsün.",
  '"Uygulanan QC düzeltmeleri" beyanına güvenme; gerçek metni kontrol et.',
  "İlk satır tam olarak: # 🔍 $BRAND_NAME — SON TEKNİK KONTROL",
]) {
  assert.ok(prepStep.includes(systemFragment), `system prompt content changed/lost: ${systemFragment}`);
}
assert.ok(
  prepStep.includes("> /tmp/final-technical-check-prompt.txt"),
...
for (const userContentFragment of [
  '"NİHAİ SENARYOLAR — " + $final_url + "\\n\\n" + $final_scripts',
  '"\\n\\nBAĞLI KALİTE KONTROL RAPORU — " + $qc_url + "\\n\\n" + $qc',
]) {
...
for (const requiredFlag of [
  ...
  "--prompt-file /tmp/final-technical-check-prompt.txt",
  "--output-file /tmp/final-check.md",
  "--meta-file /tmp/final-technical-check-meta.json",
  '--max-tokens "$FTC_MAX_MODEL_OUTPUT"',
...
for (const metaRead of [
  "FTC_AI_PROVIDER=$(jq -r '.provider // \"\"' /tmp/final-technical-check-meta.json)",
  "FTC_AI_MODEL=$(jq -r '.model // \"\"' /tmp/final-technical-check-meta.json)",
  'STOP_REASON=$(jq -r \'.stop_reason // ""\' /tmp/final-technical-check-meta.json)',
  "INPUT_TOKENS=$(jq -r '.total_input_tokens // .input_tokens // 0' /tmp/final-technical-check-meta.json)",
  "OUTPUT_TOKENS=$(jq -r '.total_output_tokens // .output_tokens // 0' /tmp/final-technical-check-meta.json)",
]) {
...
const providerGuardIdx = mustFind(
  aiStep,
  'if [[ -z "$FTC_AI_PROVIDER" || -z "$FTC_AI_MODEL" || ! -s /tmp/final-check.md ]]; then',
...
// 7. Heading contract check and the decision-contract call remain, running
// on /tmp/final-check.md (now the router's own --output-file).
mustFind(aiStep, 'EXPECTED_HEADING="# 🔍 $BRAND_NAME — SON TEKNİK KONTROL"', "heading contract check", endTurnGuardEnd);
mustFind(aiStep, "FTC_DECISION=$(node .github/scripts/final_technical_decision_contract.mjs /tmp/final-check.md)", "decision-contract call");

// 8. GITHUB_ENV carries the actual provider/model forward to the later
// report-publish step.
assert.ok(aiStep.includes('echo "FTC_AI_PROVIDER=$FTC_AI_PROVIDER"'), "FTC_AI_PROVIDER must be exported to GITHUB_ENV");
assert.ok(aiStep.includes('echo "FTC_AI_MODEL=$FTC_AI_MODEL"'), "FTC_AI_MODEL must be exported to GITHUB_ENV");
...
mustFind(
  workflow,
  '<!-- FINAL_TECHNICAL_AI_USAGE_V1 source_chars=$FTC_SOURCE_CHARS input=$FTC_INPUT_TOKENS output=$FTC_OUTPUT_TOKENS web_search=0 provider=$FTC_AI_PROVIDER model=$FTC_AI_MODEL scenarios=$FTC_SCENARIO_COUNT profile_sha=$PROFILE_FILE_SHA report_sha=$REPORT_HASH -->',
...
console.log("final_technical_check_router_migration_ok ai_calls=0 api_calls=0 issue_writes=0 dispatches=0 video_calls=0");
